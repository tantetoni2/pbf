...
func recuseRelations(id int64, handle *handler.BitmaskRelations) {
		...
		for _, member := range members {
			...
			case gosmparse.RelationType:
				if !handle.Masks.Relations.Has(member.ID) {
					...
					recuseRelations(member.ID, handle)
				}
			}
		}
	} else {
...
func BitmaskRelations(c *cli.Context) error {
	...
	handle := &handler.BitmaskRelations{
		Pass:            0,
		Mutex:           &sync.Mutex{},
