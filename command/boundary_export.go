...
func BoundaryExporter(c *cli.Context) error {
	...
	var worker = func(rel *gosmparse.Relation) {
		...
		var jsonOutput = []byte(fmt.Sprintf("{\"version\": 1,\"generator\": \"missinglink pbf\",\"osm3s\": {\"timestamp_osm_base\": \"2017-09-07T19:35:02Z\",\"copyright\": \"The data included in this document is from www.openstreetmap.org. The data is made available under ODbL.\"},\"elements\": [%s]}", strings.Join(strings.TrimSpace(strings.Split(string(json.Bytes()),"\n")), ",")))
